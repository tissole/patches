...
/* Same as get_metadata but never overwrites an existing key.
 * Used for old-style DIIN artist/title so modern ID3 wins
 * regardless of chunk order. */
static int get_metadata_no_overwrite(AVFormatContext *s,
                                     const char *const tag,
                                     const unsigned data_size)
{
    AVDictionaryEntry *e = av_dict_get(s->metadata, tag, NULL, 0);
    if (e && e->value && e->value[0])
        return 0;
    return get_metadata(s, tag, data_size);
}

/* Robust ID3 chunk handling shared by PROP-nested and top-level ID3.
 * Extracts text metadata via ff_id3v2_read plus cover (APIC) and
 * chapters. Never fails the whole file because of a bad picture;
 * only a genuine overread (id3 exceeds chunk) is an error. */
static int handle_id3_chunk(AVFormatContext *s, uint64_t size, uint64_t orig_pos)
{
    ...
    if (id3v2_extra_meta) {
        ...
        if (ret < 0)
            av_log(s, AV_LOG_WARNING, "ignoring broken APIC in ID3 chunk: %d\n", ret);
        ret = ff_id3v2_parse_chapters(s, id3v2_extra_meta);
        if (ret < 0)
            av_log(s, AV_LOG_WARNING, "ignoring broken chapters in ID3 chunk: %d\n", ret);
        ff_id3v2_free_extra_meta(&id3v2_extra_meta);
    }

    if (size < avio_tell(s->pb) - orig_pos) {
...
static int parse_dsd_diin(AVFormatContext *s, AVStream *st, uint64_t eof)
{
    ...
    while (av_sat_add64(avio_tell(pb), 12) <= eof && !avio_feof(pb)) {
        ...
        if (metadata_tag && size > 4) {
            ...
            /* Prefer ID3 over old DIIN: do not overwrite an existing
             * artist/title, and never fail open because of metadata. */
            int ret = get_metadata_no_overwrite(s, metadata_tag, FFMIN(tag_size, size - 4));
            if (ret < 0) {
                av_log(s, AV_LOG_WARNING, "ignoring broken metadata tag %s: %d\n", metadata_tag, ret);
            }
        }
...
static int parse_dsd_prop(AVFormatContext *s, AVStream *st, uint64_t eof)
{
    ...
    while (av_sat_add64(avio_tell(pb), 12) <= eof && !avio_feof(pb)) {
        ...
        case MKTAG('I','D','3',' '):
            ret = handle_id3_chunk(s, size, orig_pos);
            if (ret < 0)
                return ret;
            break;
...
static int iff_read_header(AVFormatContext *s)
{
    ...
    while(!avio_feof(pb)) {
        ...
        case MKTAG('C','O','M','T'):
            ...
            for (i = 0; i < nb_comments; i++) {
                ...
                if ((res = get_metadata(s, tag, metadata_size)) < 0) {
                    av_log(s, AV_LOG_WARNING, "ignoring broken comment %s: %d\n", tag, res);
                    /* Stop parsing comments, outer skip jumps to next chunk. */
                    break;
                }
        ...
        case MKTAG('I','D','3',' '):
            res = handle_id3_chunk(s, data_size, orig_pos);
            if (res < 0)
                return res;
            break;
        }
        ...
        if (metadata_tag) {
            /* Old text tags: do not overwrite modern ID3 values. */
            if ((res = get_metadata_no_overwrite(s, metadata_tag, data_size)) < 0) {
                av_log(s, AV_LOG_WARNING, "ignoring broken tag %s: %d\n", metadata_tag, res);
            }
        }
    ...
    if (sta) {
        ...
        /* Duration fallback for uncompressed audio (plain DSD/PCM):
         * DST is variable-size and uses FRTE instead, so skip it here.
         * total_bits / bits_per_second * sample_rate = samples. */
        if (!sta->duration && sta->codecpar->codec_tag != ID_DST &&
            iff->body_size > 0 && sta->codecpar->bit_rate > 0 &&
            sta->codecpar->sample_rate > 0) {
            sta->duration = av_rescale(iff->body_size,
                                       8LL * sta->codecpar->sample_rate,
                                       sta->codecpar->bit_rate);
        }
    }
